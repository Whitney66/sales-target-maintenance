...
function createFormLine(data = {}, canRemove = false) {
  ...
  return `<div class="add-row" data-form-line>
    ...
    <label class="form-field"><strong><i>*</i>员工姓名 / 工号</strong><div class="employee-combobox"><input data-field="employeeText" autocomplete="off" placeholder="搜索或选择员工" value="${escapeHtml(employeeText)}" /><button type="button" data-employee-trigger aria-label="展开员工列表">⌄</button><div class="employee-options">${employees.map(employee => `<button type="button" data-employee-value="${escapeHtml(`${employee.name} / ${employee.id}`)}">${escapeHtml(employee.name)} <span>/ ${employee.id}</span></button>`).join("")}</div></div><small><b>校验规则：</b>必填，单选，取自EOP，展示为“姓名 / 8位工号”。</small></label>
...
$("#formLines").addEventListener("click", event => {
  ...
  if (employeeTrigger || employeeInput) { event.preventDefault(); event.stopPropagation(); const combo = event.target.closest(".employee-combobox"); document.querySelectorAll(".employee-combobox.open").forEach(item => { if (item !== combo) item.classList.remove("open"); }); if (employeeTrigger) combo.classList.toggle("open"); else combo.classList.add("open"); return; }
